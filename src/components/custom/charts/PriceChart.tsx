...
function generatePriceData(initialPrice, currentPrice, period) {
  const dataPoints = [];
  let numDataPoints;
  ...
  for (let i = 0; i < numDataPoints; i++) {
    ...
    dataPoints.push({
      name: date,
      value: newPrice.toFixed(2), // Round to 2 decimals
    });
  }
...
function PriceChart({ initialPrice, currentPrice }) {
      ...
      <div className="flex flex-row items-center justify-between">
        ...
        <div className="flex flex-row items-center gap-x-3">
          <button
            ...
            className={
              period === "1m"
                ? "px-4 py-1 text-sm font-bold rounded-lg  bg-alpha text-beta border-0 border-black"
      ...
      <div className="pt-1 pr-6 mb-6 rounded-lg bg-">
        <ResponsiveContainer width="100%" height={250}>
          <LineChart data={data}>
            ...
            <XAxis dataKey="name" />
            <YAxis domain={[minValue, currentPrice + 10]} />
            <Tooltip />
            <Line
              type="monotone"
              dataKey="value"
